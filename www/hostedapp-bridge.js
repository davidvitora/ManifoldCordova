...
(function(platform, pluginMode, cordovaBaseUrl) {
  function onCordovaLoaded() {
    ...
    channel.onNativeReady.subscribe(function() {
      ...
      if (platform === "ios") {
        ...
        if (
          exec.setJsToNativeBridgeMode &&
          exec.jsToNativeModes &&
          exec.jsToNativeModes.XHR_OPTIONAL_PAYLOAD
        ) {
          exec.setJsToNativeBridgeMode(
            exec.jsToNativeModes.XHR_OPTIONAL_PAYLOAD
          );
      ...
      pluginloader.injectScript = function(url, onload, onerror) {
        ...
        if (
          url.indexOf(
            cordovaPluginsScript,
            url.length - cordovaPluginsScript.length
          ) !== -1
        ) {
  ...
  window.hostedWebApp.platform,
  window.hostedWebApp.pluginMode,
  window.hostedWebApp.cordovaBaseUrl
);
